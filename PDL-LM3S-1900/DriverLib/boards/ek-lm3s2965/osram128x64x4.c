...
//        When Enabled, ROW 0-79 map to COM 79-0 (that is, reverse row order)
// Bit 3: Reserved
...
OSRAM128x64x4Clear(void)
{
    ...
    *(unsigned long *)&g_pucBuffer[4] = 0;
    ...
    for(ulColumn = 0; ulColumn < (128 / 2); ulColumn++)
    {
        ...
        {
            OSRAMWriteData(g_pucBuffer, sizeof(g_pucBuffer));
        }
    }
}
                        ...
                        unsigned long ulY, unsigned char ucLevel)
{
    ...
    g_pucBuffer[1] = ulX / 2;
    g_pucBuffer[2] = 63;
    OSRAMWriteCommand(g_pucBuffer, 3);
    g_pucBuffer[0] = 0x75;
    g_pucBuffer[1] = ulY;
    g_pucBuffer[2] = ulY + 7;
    OSRAMWriteCommand(g_pucBuffer, 3);
