...
//*****************************************************************************
//
// The point accumulated during the game.
//
//*****************************************************************************
...
// 0 to 767.  Each coordinate must be an even number to avoid having to shift
...
DrawMaze(void)
{
    ...
    //
    for(lY1 = lYPos; lY1 < 64; lY1 += 12, lYCell++)
    {
        //
        // Do not draw this cell if it is not part of the maze.
        //
        if((lYCell < 0) || (lYCell > 63))
        {
            continue;
        }
        ...
        for(lX1 = lXPos; lX1 < 128; lX1 += 12, lXTemp++)
        {
            ...
            // the display.
            //
            for(lY2 = (lY1 < 0) ? -lY1 : 0; (lY2 < 12) && ((lY1 + lY2) < 64);
                lY2++)
            {
                //
...
DrawBullets(tBoolean bFire)
{
    ...
    for(ulLoop = 0; ulLoop < 4; ulLoop++)
    {
        ...
        // be unfired.
        //
        if((g_pusBulletX[ulLoop] > 1524) || (g_pusBulletY[ulLoop] > 768))
        {
            g_pusBulletX[ulLoop] = 0;
            g_pusBulletY[ulLoop] = 0;
        }
        ...
        if((g_pusBulletX[ulLoop] != 0) || (g_pusBulletY[ulLoop] != 0))
        {
            ...
            lY = (long)g_pusBulletY[ulLoop] - (long)g_usPlayerY + (32 - 6);

            //
            // See if the bullet is completely off the display.
            //
            if((lX < -3) || (lX > 127) || (lY < -3) || (lY > 63))
            {
                //
...
DrawPlayer(unsigned char ucDirection)
{
    ...
    for(ulLoop = 0; ulLoop < 72; ulLoop++)
    {
        ...
        // collision.
        //
        ucData1 = g_pucFrame[(((ulLoop / 6) + 26) * 64) + (ulLoop % 6) + 29];
        ...
        g_pucFrame[(((ulLoop / 6) + 26) * 64) + (ulLoop % 6) + 29] =
            ucData1 | ucData2;
    }
...
MainScreen(void)
{
    ...
    for(ulCount = 0; ulCount < (2 * 60 * 30); ulCount++)
    {
        ...
        // maze.
        //
        else if((g_usPlayerY == (768 - 102)) && (g_usPlayerX != 90))
        {
            //
        ...
        //
        for(ulLoop = 0; ulLoop < 4096; ulLoop += 4)
        {
        ...
        if(ulCount & 16)
        {
            ...
            for(ulLoop = 0; ulLoop < (64 * 16); ulLoop++)
            {
                ...
                //
                g_pucFrame[ulLoop + (24 * 64)] = g_pucPlay[ulLoop];
            }
        }
        ...
        if(ulAddr == 0)
        {
            ...
            for(ulLoop = 0; ulLoop < (64 * 13); ulLoop++)
            {
                ...
                // Copy this byte from the image to the local frame buffer.
                //
                g_pucFrame[ulLoop + (51 * 64)] = g_pucAcquiring[ulLoop];
            }
        }
        else
        {
            ...
            //
            ulStart = (51 * 64) + ((128 - (ulStart + 30)) / 4);
        ...
        //
        OSRAM128x64x4ImageDraw(g_pucFrame, 0, 0, 128, 64);
    }

    //
    // Clear out the maze.
    //
    for(ulLoop = 0; ulLoop < 64; ulLoop++)
    {
...
PlayGame(void)
{
    ...
    while(1)
    {
            ...
            for(ulLoop = 0; ulLoop < 100; ulLoop++)
            {
                ...
                do
                {
                    ...
                    g_pusMonsterY[ulLoop] =
                        ((((RandomNumber() / 65536) * 21) / 65536) * 36) + 18;
                }
        ...
        else
        {
            ...
            {
                g_pucFrame[(((ulLoop / 22) + 46) * 64) +
                           (ulLoop % 22) + 8] = g_pucScore[ulLoop];
            }
            for(ulLoop = 0; ulLoop < 13; ulLoop++)
            {
                g_pucFrame[((ulLoop + 46) * 64) + 30] = 0;
            }
            DrawNumber((46 * 64) + 31, g_ulScore % 1000000, 100000, false);
