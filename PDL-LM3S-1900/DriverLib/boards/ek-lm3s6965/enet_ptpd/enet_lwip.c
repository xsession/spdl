...
//*****************************************************************************
//
// Timestamp for lwIP/PTPd.
//
//*****************************************************************************
static TimeInternal tRxTimestamp;
...
//*****************************************************************************
void
getRxTime(TimeInternal *tRxTime)
{
    //
    ...
    //
    // Check to see if we have an RX Timestamp and use it instead.
    //
    if(tRxTimestamp.seconds)
    {
        tRxTime->seconds = tRxTimestamp.seconds;
        tRxTime->nanoseconds = tRxTimestamp.nanoseconds;
        tRxTimestamp.seconds = tRxTimestamp.nanoseconds = 0;
    }
}
...
EthernetIntHandler(void)
{
    ...
    {
        //
        // Read and adjust the Rx Timestamp.
        //
        if(!tRxTimestamp.seconds)
        {
            getTime(&tRxTimestamp);
        }
                     ...
                     unsigned long ulRow)
{
    ...
    // Display the string.
    //
    OSRAM128x64x4StringDraw(pucBuf, ulCol, ulRow, 15);
}
...
lwip_init(void)
{
    ...
    OSRAM128x64x4StringDraw("Waiting for DHCP", 0, 16, 15);
    OSRAM128x64x4StringDraw("<                   > ", 0, 24, 15);
...
//*****************************************************************************
//
// Should be called by the top-level application every system tick, with
// the number of MS per system tick, to run lwIP timers, etc.
//
//*****************************************************************************
void
lwip_tick(unsigned long ulTickMS)
{
    ...
    if(g_sEMAC_if.dhcp != NULL)
    {
        ...
        else
        {
            ...
            {
                OSRAM128x64x4StringDraw("< ", 0, 24, 15);
                OSRAM128x64x4StringDraw("*",iColumn, 24, 7);
            }
            else
            {
                OSRAM128x64x4StringDraw(" *",iColumn - 6, 24, 7);
            }
            ...
            {
                iColumn = 6;
                OSRAM128x64x4StringDraw(" >", 114, 24, 15);
            }
        }
    }
    ...
    if(bDisplayIP && !bIPDisplayed)
    {
        ...
        OSRAM128x64x4StringDraw("                       ", 0, 16, 15);
        OSRAM128x64x4StringDraw("                       ", 0, 24, 15);
        OSRAM128x64x4StringDraw("IP:   ", 0, 16, 15);
        OSRAM128x64x4StringDraw("MASK: ", 0, 24, 15);
        OSRAM128x64x4StringDraw("GW:   ", 0, 32, 15);
