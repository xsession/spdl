...
// 2000.0000 - 2000.0400 - rgn 1: read-write, RAM
// 2000.0400 - 2000.0500 - rgn 2: read-only, RAM (disabled sub-rgn 4 of rgn 1)
// 2000.0500 - 2000.0800 - rgn 1: read-write, RAM
...
main(void)
{
    ...
    //
    // Configure a read-write MPU region for RAM.  It is a 2 KB region.  There
    // is a 256 byte sub-region in the middle that is disabled in order to open
    // up a hole in which different permissions can be applied.
    //
    ...
    //
    // Configure a read-only MPU region for the 256 bytes of RAM that are
    // disabled in the previous region.  This region is used for demonstrating
    // read-only permissions.
    //
    MPURegionSet(2, SRAM_BASE + 0x400,
                 MPU_RGN_SIZE_256B |
                 MPU_RGN_PERM_NOEXEC |
                 MPU_RGN_PERM_PRV_RO_USR_RO |
                 MPU_RGN_ENABLE);
    ...
    //
    // Attempt to read from the read-only area of RAM, the middle 256 B of the
    // 2 KB region.
    //
    g_ulMPUFaultCount = 0;
    g_ulValue = HWREG(0x20000440);
    ...
    //
    // Attempt to write to the read-only area of RAM, the middle 256 B of the
    // 2 KB region.
    //
    g_ulMPUFaultCount = 0;
    HWREG(0x20000460) = 0xabcdef00;
