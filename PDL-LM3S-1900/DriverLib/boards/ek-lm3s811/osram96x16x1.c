//*****************************************************************************
//
// osram96x16x1.c - Driver for the OSRAM 96x16 monochrome graphical OLED
//                  display.
//
...
#if defined(ewarm)
static void
OSRAM96x16x1Delay(unsigned long ulCount)
{
    __asm("    subs    r0, #1\n"
          "    bne     OSRAM96x16x1Delay\n"
          "    bx      lr");
}
#endif
...
//*****************************************************************************
void
OSRAM96x16x1ImageDraw(const unsigned char *pucImage, unsigned long ulX,
                      ...
                      unsigned long ulHeight)
{
    ...
    while(ulHeight--)
    {
        ...
        OSRAM96x16x1WriteByte((ulY == 0) ? 0xb0 : 0xb1);
        OSRAM96x16x1WriteByte(0x80);
        OSRAM96x16x1WriteByte(ulX & 0x0f);
        OSRAM96x16x1WriteByte(0x80);
        OSRAM96x16x1WriteByte(0x10 | ((ulX >> 4) & 0x0f));
        OSRAM96x16x1WriteByte(0x40);

        //
        // Write this row of image data.
        //
        OSRAM96x16x1WriteArray(pucImage, ulWidth - 1);
        OSRAM96x16x1WriteFinal(pucImage[ulWidth - 1]);
...
OSRAM96x16x1Init(tBoolean bFast)
{
    ...
    // OSRAM96x16x1Delay(1700) for a 100 kHz I2C bus with the CPU running at 50
    // MHz (referred to as C).  To scale this to the delay for a different CPU
        ...
        ulIdx += g_pucOSRAMInit[ulIdx] + 1)
    {
        ...
        // Send this command.
        //
        OSRAM96x16x1WriteFirst(g_pucOSRAMInit[ulIdx + 1]);
        OSRAM96x16x1WriteArray(g_pucOSRAMInit + ulIdx + 2,
                               g_pucOSRAMInit[ulIdx] - 2);
        OSRAM96x16x1WriteFinal(g_pucOSRAMInit[ulIdx + g_pucOSRAMInit[ulIdx]]);
    }
...
OSRAM96x16x1DisplayOff(void)
{
    ...
    //
    OSRAM96x16x1WriteFirst(0x80);
    OSRAM96x16x1WriteByte(0xae);
    OSRAM96x16x1WriteByte(0x80);
    OSRAM96x16x1WriteByte(0xad);
    OSRAM96x16x1WriteByte(0x80);
    OSRAM96x16x1WriteFinal(0x8a);
}
