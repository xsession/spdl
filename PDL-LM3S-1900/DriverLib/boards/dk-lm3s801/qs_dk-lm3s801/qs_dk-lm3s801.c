//*****************************************************************************
//
// qs_dk-lm3s801.c - A quick start sample application to demo chip features
//
...
//
//*****************************************************************************
#define COMPARATOR_ID   1
#define GPIO_PIN_MUTE   GPIO_PIN_4

//*****************************************************************************
//
//! \addtogroup dk_lm3s801_list
//! <h1>DK-LM3S801 Quickstart Application (qs_dk-lm3s801)</h1>
//!
//! This example uses the potentiometer on the development board to vary the
//! rate and frequency of a repetitive beep from the piezo buzzer.  Turning the
//! knob in one direction will result in slower beeps at lower frequency, while
//! turning it the other direction will result in faster beeps at a higher
//! frequency.  The potentiometer setting along with the tone ``note'' is
//! displayed on the LCD, and a log of the readings is output on the UART at
//! 115,200, 8-n-1.  The push button can be used to turn the beeping noise on
//! and off; when off the LCD and UART still show the setting.
//
//*****************************************************************************
...
SysTickIntHandler(void)
{
    ...
    //
    // Determine the tone frequency of the piezo buffer.  The input for tone
    // level is the comparator.  Look up the tone freq based on 1 of 16 levels.
    //
    g_ulToneFreq = g_ulFreqMap[g_ulPotSetting];
    g_chNote = g_chNoteMap[g_ulPotSetting];
...
main(void)
{
    ...
    //
    // Set GPIO B5 as an analog comparator input.
    //
    GPIOPinTypeComparator(GPIO_PORTB_BASE, GPIO_PIN_5);
