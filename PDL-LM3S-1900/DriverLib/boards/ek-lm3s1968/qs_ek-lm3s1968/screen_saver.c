...
ScreenSaver(void)
{
    ...
    //
    // The screen saver timed out, so configure the hibernation module to wake
    // on pin, and put the part in hibernation mode.
    //
    HibernateWakeSet(HIBERNATE_WAKE_PIN);
    HibernateRequest();

    //
    // The processor will continue to execute for a few more cycles until the
    // hibernation module removes power.  If for some reason the processor does
    // not hibernate, then the code below will turn off the display, and wait
    ...
    PWMGenPeriodSet(PWM_BASE, PWM_GEN_0, g_ulSystemClock / 10000);
