...
//*****************************************************************************
//
// The high score that was saved in battery backed memory.  The high score
// value is saved at index [1].  Indices [0] and [2] are used to hold
// magic numbers to indicate the validity of the saved data.
//
//*****************************************************************************
unsigned long g_ulHighScore[3];

//*****************************************************************************
//
// A flag to indicate that a new high score was achieved.
//
//*****************************************************************************
unsigned char g_bNewHighScore = 0;

//*****************************************************************************
//
// Magic numbers that are used to validate the high score value in
// battery backed memory.
//
//*****************************************************************************
#define SCOREVALID1     0x68676948
#define SCOREVALID2     0x726F6353

//*****************************************************************************
//
// Points to the start of the last flash sector.
//
//*****************************************************************************
static volatile unsigned long *pulFlash = (volatile unsigned long *)0x0003FC00;
    ...
    for(ulLoop = 0; ulLoop < 4; ulLoop++)
    {
        ...
        if((g_pusBulletX[ulLoop] != 0) || (g_pusBulletY[ulLoop] != 0))
        {
            ...
            if(bHit)
            {
                ...
                //
                ClassDStop();
                ClassDPlayADPCM(g_pucMonsterEffect,
                                sizeof(g_pucMonsterEffect));
...
MainScreen(void)
{
    ...
    //
    // Read the high score from the battery backed memory in the hibernation
    // module.  The first 3 locations are used to save the score.
    //
    HibernateDataGet(g_ulHighScore, 3);

    //
    // Validate the high score that was read from the NV memory.  The first and
    // third locations should contain certain magic numbers.  If these values
    // are not correct, then the saved high score at the second location should
    // be assumed to not be valid, and is reset to 0.
    //
    if((g_ulHighScore[0] != SCOREVALID1) || (g_ulHighScore[2] != SCOREVALID2))
    {
        g_ulHighScore[1] = 0;

        //
        // The following code block is needed due to hibernation module errata.
        // When the errata is removed, this code block can be deleted.  The
        // hibernation module may have been reset, losing the high score.
        // Therefore, also check the flash to see if a high score was saved
        // there.
        //
        // Check the beginning of the high flash sector to see if appears to
        // have high score data there.  The first two locations will be marked
        // with a certain value.
        //
        if((pulFlash[0] == SCOREVALID1) && (pulFlash[1] == SCOREVALID2))
        {
            //
            // It appears high score data is stored in flash, so search the
            // last sector for the high score.
            //
            for(ulLoop = 2; ulLoop < 256; ulLoop++)
            {
                //
                // If a blank location is found, assume the remainder of the
                // block is blank and stop looking.
                //
                if(pulFlash[ulLoop] == 0xFFFFFFFF)
                {
                    break;
                }

                //
                // If there is a non-blank value here, then assume it is a
                // saved high score.  Assume the scores always increase in the
                // storage order, so the last value found is the highest.
                //
                else
                {
                    g_ulHighScore[1] = pulFlash[ulLoop];
                }
            }
        }
    }
    ...
    for(ulCount = 0; ulCount < (2 * 60 * 30); ulCount++)
    {
        ...
        //
        // Display the high score message.  Loop over the bytes in the image.
        //
        for(ulLoop = 0; ulLoop < 494; ulLoop++)
        {
            g_pucFrame[(((ulLoop / 38) + 5) * 64) + (ulLoop % 38) + 0] =
                g_pucHighScore[ulLoop];
        }

        //
        // Display the high score value.
        //
        for(ulLoop = 0; ulLoop < 13; ulLoop++)
        {
            g_pucFrame[((ulLoop + 5) * 64) + 38] = 0;
        }
        DrawNumber((5 * 64) + 39, g_ulHighScore[1] % 1000000, 100000, false);
...
PlayGame(void)
{
    ...
    //
    // Reset the high score indicator.
    //
    g_bNewHighScore = 0;
    ...
    //
    ClassDStop();
    ClassDPlayADPCM(g_pucStartOfGame, sizeof(g_pucStartOfGame));
    ...
    while(1)
    {
        ...
        if(!bDead && !bAtExit)
        {
            ...
            else
            {
                   ...
                   (g_usPlayerY < 6) || (g_usPlayerY > 1110))
                {
                    ...
                    //
                    ClassDStop();
                    ClassDPlayADPCM(g_pucEndOfMaze, sizeof(g_pucEndOfMaze));
                }
                ...
                if(bDead)
                {
                    ...
                    //
                    ClassDStop();
                    ClassDPlayADPCM(g_pucPlayerEffect,
                                    sizeof(g_pucPlayerEffect));
        ...
        {
            //
            // Check to see if this is a new high score, and if so save
            // it into the battery backed memory.  Also set two magic
            // numbers to indicate the stored data is valid.
            //
            if(g_ulScore > g_ulHighScore[1])
            {
                g_ulHighScore[1] = g_ulScore;
                g_ulHighScore[0] = SCOREVALID1;
                g_ulHighScore[2] = SCOREVALID2;
                HibernateDataSet(g_ulHighScore, 3);

                //
                // Set indicator for a new high score, so that a message
                // can be displayed later.
                //
                g_bNewHighScore = 1;

                //
                // The following code block is needed due to hibernation module
                // errata.  When the errata is removed, this code block can be
                // deleted.  The hibernation module might get reset, losing
                // the high score.  Therefore, also save the high score in
                // the last sector of flash.
                //
                // Check the beginning of the high flash sector to see if
                // high scores are already being stored there.  There will
                // be special numbers in the first two locations.  Also
                // make sure that there is at least one blank location where
                // a score can be stored.
                //
                if((pulFlash[0] != SCOREVALID1) ||
                   (pulFlash[1] != SCOREVALID2) ||
                   (pulFlash[255] != 0xFFFFFFFF))
                {
                    //
                    // Magic numbers were not found in the last sector of
                    // flash and so it is not already being used for high
                    // score.  Therefore it needs to be erased and magic
                    // numbers stored before a score can be stored there.
                    //
                    FlashErase((unsigned long)&pulFlash[0]);
                    FlashProgram(&g_ulHighScore[0],
                                 (unsigned long)&pulFlash[0], 4);
                    FlashProgram(&g_ulHighScore[2],
                                 (unsigned long)&pulFlash[1], 4);
                }

                //
                // Search for the next available blank location in the
                // flash sector, and store the high score there.
                //
                for(ulLoop = 2; ulLoop < 256; ulLoop++)
                {
                    if(pulFlash[ulLoop] == 0xFFFFFFFF)
                    {
                        FlashProgram(&g_ulHighScore[1],
                                     (unsigned long)&pulFlash[ulLoop], 4);
                        break;
                    }
                }
            }
            ...
            if(ulCount == 130)
            {
                ...
                //
                ClassDStop();
                ClassDPlayADPCM(g_pucEndOfGame, sizeof(g_pucEndOfGame));
            }
            ...
            //
            // If a new high score was set, display the "New high score"
            // message.
            //
            if(g_bNewHighScore)
            {
                for(ulLoop = 0; ulLoop < 676; ulLoop++)
                {
                    g_pucFrame[(((ulLoop / 52) + 40) * 64) +
                               (ulLoop % 52) + 6] = g_pucNewHighScore[ulLoop];
                }
            }
